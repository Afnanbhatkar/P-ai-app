...
function App() {
  ...
  const sendMessage = async (textOverride = null) => {
    ...
    if (!user && loggedIn) {
      setTimeout(() => {
        addBotMessage(`(guest mode) I heard: "${text}" — try signing in to use full features.`);
        setSending(false);
      }, 600);
      return;
    }
  ...
  try {
    ...
    // set chats as returned by server (we expect newest-first ideally)
    setChats(data);

    // if a currentChatId is already selected and still exists on server, keep it
    if (currentChatId) {
      const existing = data.find((c) => c.id === currentChatId);
      if (existing) {
    ...
    // otherwise pick the first chat (newest) if available
    if (data.length > 0) {
      const latest = data[0];
...
const handleNewChat = async () => {
  // Local guest path: create and prepend
  if (!user) {
    const chatId = Date.now();
    const greetingName = username || "there";
    const botGreeting = `Hi ${greetingName}! 👋 I'm Jarvis — your assistant. How can I help you today?`;
    ...
    // prepend so list order keeps newest first
  ...
  // Authenticated path: call server and prepend result
  setSending(true);
  try {
    const token = await getIdToken();
    ...
    if (res.ok && data.chat) {
      // put server chat at front (avoid duplicates)
      setChats((prev) => {
      ...
      setShowHowCan(false);
      // optionally update stored chats from server but don't overwrite selection
      // await loadChats(); // NOT necessary — we already inserted the chat
    } else {
    ...
    <div className="min-h-screen w-full overflow-hidden bg-white relative font-sans font-medium">
      ...
      <div className="fixed top-4 right-4 z-[60] flex flex-col gap-2">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={`max-w-sm px-4 py-2 rounded-md shadow-md text-sm ${
              t.type === "error"
      ...
      {!appVisible && (
        <>
          ...
          <div
            className={`fixed inset-y-0 left-0 w-full md:w-3/5 z-50 transition-all duration-1000 ease-in-out transform-gpu no-seam ${
              showLoginSlide ? "translate-x-0" : "-translate-x-full"
            }`}
            style={{
              background:
          ...
          >
            <div className="absolute inset-0 flex flex-col items-start justify-center px-10 md:px-20">
              <h1 className="text-5xl md:text-6xl font-bold text-orange-600 mb-8">P-ai ●</h1>
          ...
          <div
            className={`fixed bg-[#f9f7f5] inset-y-0 right-0 w-full md:w-2/5 z-50 flex items-center justify-center transition-all duration-1000 delay-200 ease-in-out transform-gpu ${
              showLoginSlide ? "translate-x-0" : "translate-x-full"
            }`}
          ...
          >
            <div className="bg-[#f9f7f5] rounded-2xl shadow-xl p-8 w-full max-w-sm mx-4">
              ...
              <button
                ...
                className={`w-full py-3 rounded-xl font-semibold text-white shadow-sm transition transform active:scale-95 ${
                  !tempName.trim() || sending
              ...
              <form onSubmit={handleEmailAuth} className="space-y-3">
                ...
                <button
                  type="submit"
                  disabled={sending}
                  className={`w-full mt-1 py-3 rounded-xl font-semibold text-[#d8bfa1] transition active:scale-95 ${
                    sending ? "bg-gray-300 text-gray-600 cursor-not-allowed" : "bg-orange-100 hover:bg-[#e6cfb3] hover:text-[#faf4ed]"
                  }`}
                  aria-busy={sending}
                >
              ...
              <button
                ...
                disabled={sending}
                className={`w-full mt-3 flex items-center justify-center gap-3 py-3 rounded-xl font-medium transition ${
                  sending ? "opacity-60 cursor-not-allowed" : "bg-white shadow-sm hover:shadow-md"
                }`}
              ...
              <p className="mt-6 text-xs text-gray-400 text-center">P-ai is powered by DeepAI + Firebase • Made by Afnan</p>
            </div>
          </div>
        </>
      )}
      ...
      {appVisible && (
        <>
          {/* Sidebar */}
          <div
            className={`fixed top-0 left-0 h-full w-64 bg-[#fbf4e9] z-40 transform transition-transform duration-500 ease-in-out shadow-sm ${
          ...
          >
            ...
            <div className="flex flex-col h-full">
              <div className="px-6 pt-6 pb-4 flex items-center gap-3">
                ...
                <button className="text-sm text-[#8b7c63] px-2 py-1 rounded hover:bg-[#f3ead8]">☰</button>
              </div>
                        ...
                        return (
                          <div
                            ...
                            onClick={() => {
                              ...
                              setMessages((chat.messages || []).map((m) => ({ from: m.role === "user" ? "user" : "bot", text: m.content })));
                              setShowHowCan(false);
                              setOpenMenuId(null);
                              setRenameEditingId(null);
                              setDeleteConfirmId(null);
                            }}
                            className={`relative flex items-start gap-3 px-3 py-2 rounded-lg cursor-pointer transition select-none truncate ${
                              chat.id === currentChatId ? "bg-[#f3e9d4] text-[#1f2b1f] font-medium shadow-inner" : "hover:bg-[#fbf7ef] text-[#3a352b]"
                            }`}
                          >
              ...
              <div className="max-w-4xl mx-auto">
                <div className="bg-[#faf5ef] rounded-3xl p-5 flex gap-4 items-center backdrop-blur-sm transition-all duration-300 shadow-lg border border-transparent hover:shadow-none hover:border-[#dec19d] focus-within:border-[#dec19d] focus-within:shadow-none">
                  ...
                  <button
                    ...
                    className={`w-14 h-14 rounded-full flex items-center justify-center transition-all duration-300 shadow-md hover:shadow-lg active:scale-105 ${
                      !input.trim() ? "bg-[#f5ede4] text-[#c7b29e] shadow-inner" : "bg-emerald-600 text-white shadow-lg hover:bg-emerald-700"
                    }`}
                  >
